...
# ========== 配置 ==========
EXISTING_RESULTS_DIR = r"d:\Pycharm\PycharmProjects\joint_sensitivity_v2\results_20260323_145643"
...
def continue_optimization(existing_csv_path, baseline_sales, baseline_structure, output_dir):
    ...
    # 准备训练数据（5维：add_cap, R_total, R_pick, B_total, B_pick）
    X_train = existing_df[['add_cap', 'R_total', 'R_pick', 'B_total', 'B_pick']].values
    ...
    best_params = best_row[['add_cap', 'R_total', 'R_pick', 'B_total', 'B_pick']].values
    ...
    for iteration in range(CONTINUE_ITERATIONS):
        ...
        # 添加新数据到训练集
        new_points = candidate_results[['add_cap', 'R_total', 'R_pick', 'B_total', 'B_pick']].values
        ...
        if len(new_scores) > 0:
            ...
            if current_best_score > best_score:
                ...
                print(f"   ★ NEW BEST: add_cap={add_cap:.1f}, 红球={int(R_total)}选{int(R_pick)}, 蓝球={int(B_total)}选{int(B_pick)}, p={best_p:.2e}, 耗时={candidate_elapsed:.1f}s, 综合={best_score:.4f}(结构{best_structure:.4f},销量{best_sales:.4f})")
            else:
...
def generate_visualizations(all_results_df, output_dir):
    ...
    plt.title('Bayesian Optimization Convergence - Joint (Continued)')
    ...
    plt.title('Pareto Frontier: Sales vs Structure - Joint (Continued)')
    plt.grid(True, linestyle='--', alpha=0.6)
    ...
    plt.title('Correlation Heatmap - Joint (Continued)')
    plt.tight_layout()
    ...
    try:
        ...
        thesis_3d_path = os.path.join(thesis_plots_dir, "fig_joint_sensitivity_3d_continued.png")
        fig = plt.figure(figsize=(18, 6))

        ax1 = fig.add_subplot(131, projection='3d')
        scatter1 = ax1.scatter(all_results_df['add_cap'], np.log10(all_results_df['p'].replace(0, np.nan)),
        ...
        scatter2 = ax2.scatter(all_results_df['add_cap'], np.log10(all_results_df['p'].replace(0, np.nan)),
        ...
        scatter3 = ax3.scatter(all_results_df['add_cap'], np.log10(all_results_df['p'].replace(0, np.nan)),
...
def main():
    ...
    # 找到最新的CSV结果
    existing_csv = os.path.join(EXISTING_RESULTS_DIR, "joint_results_20260323_145643.csv")
    ...
    results_csv_path = os.path.join(output_dir, f"joint_results_continued_{timestamp}.csv")
    ...
    if len(positive_structure) > 0:
        ...
        print(f"   最优正值结构解：add_cap={best_positive['add_cap']:.1f}, "
              f"红球={int(best_positive['R_total'])}选{int(best_positive['R_pick'])}, "
