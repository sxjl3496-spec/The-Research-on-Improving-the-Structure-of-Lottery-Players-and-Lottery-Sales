...
# ========== 配置 ==========
# 使用之前运行的结果目录
EXISTING_RESULTS_DIR = r"d:\Pycharm\PycharmProjects\cap_only_sensitivity_v2\results_20260323_144716"
...
def generate_visualizations(all_results_df, output_dir):
    ...
    plt.title('Bayesian Optimization Convergence Curve - Cap Only (Continued)')
    ...
    plt.title('Pareto Frontier: Sales vs Structure - Cap Only (Continued)')
    plt.grid(True, linestyle='--', alpha=0.6)
    ...
    axes[1, 1].scatter(all_results_df['add_cap'], all_results_df['y_structure'], alpha=0.6)
    axes[1, 1].set_xlabel('Add Cap (万元)')
    axes[1, 1].set_ylabel('Normalized Structure Score (y_structure)')
    axes[1, 1].set_title('Add Cap vs Structure Score')
    ...
    # (c) 结构指数
    axes[2].scatter(all_results_df['add_cap'], all_results_df['y_structure'], alpha=0.6, s=30)
    axes[2].set_xlabel('封顶额 (万元)')
    axes[2].set_ylabel('彩民结构指数得分 (y_structure)')
    axes[2].set_title('(c) 封顶额 vs 彩民结构指数得分')
...
def continue_optimization(existing_csv_path, baseline_sales, baseline_structure, output_dir):
    ...
    # 准备训练数据
    X_train = existing_df[['add_cap']].values
    ...
    best_params = [best_row['add_cap']]
    best_structure = best_row['y_structure']
    best_sales = best_row['y_sales']

    print(f"\n 继续贝叶斯优化（{CONTINUE_ITERATIONS}次迭代）...")

    best_history = [{'iteration': 0, 'best_score': best_score,
                     'best_add_cap': best_params[0],
                     'y_structure': best_structure, 'y_sales': best_sales}]
    ...
    for iteration in range(CONTINUE_ITERATIONS):
        ...
        # 添加新数据到训练集
        new_points = candidate_results[['add_cap']].values
        ...
        if len(new_scores) > 0:
            ...
            if current_best_score > best_score:
                ...
                print(f"   ★ NEW BEST: add_cap={add_cap:.1f}, 耗时={candidate_elapsed:.1f}s, 综合={best_score:.4f}(结构{best_structure:.4f},销量{best_sales:.4f})")
            else:
...
def main():
    print("\n" + "=" * 80)
    print(" 博士论文：彩票封顶额单一参数敏感性分析 - 断点续跑")
    print("=" * 80)
    ...
    # 找到最新的CSV结果
    existing_csv = os.path.join(EXISTING_RESULTS_DIR, "cap_only_results_20260323_144716.csv")
    ...
    results_csv_path = os.path.join(output_dir, f"cap_only_results_continued_{timestamp}.csv")
